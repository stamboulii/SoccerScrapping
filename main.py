...
try:
    ...
    from database.postgres_connection import db_manager, init_database, generate_sample_data
    LOGURU_AVAILABLE = True
except ImportError:
    LOGURU_AVAILABLE = False
    import logging
...
async def run_async_scraping():
    ...
    async with AsyncSoccerScraper(max_concurrent=5) as scraper:
        ...
        # 👇 Exemple d'insertion si les données sont au bon format
        player_data = []
        for item in data:
            try:
                name = item.get("name")
                ...
                club = item.get("club")
                if name and age > 0 and club:
                    player_data.append((name, age, club))
            except Exception as e:
                print(f"⚠️ Invalid data skipped: {e}")
        
        if player_data:
            db_manager.bulk_insert_players(player_data)
